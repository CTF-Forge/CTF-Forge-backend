...
- **チャレンジの編集**
    - <del>**エンドポイント**: `PUT /api/challenges/:challengeId`</del>
    - **目的**: 既存のチャレンジ（タイトル、説明、カテゴリなど）を更新します。
    - **検証**: リクエストを行ったユーザーがチャレンジの所有者であることを検証します。

- **チャレンジの削除**
    - <del>**エンドポイント**: `DELETE /api/challenges/:challengeId`</del>
    - **目的**: プラットフォームからチャレンジを削除します。
    - **検証**: チャレンジの所有者のみが削除できるようにします。

- **特定のチャレンジ詳細の取得（作成者用）**
    - <del>**エンドポイント**: `GET /api/challenges/:challengeId`</del>
...
- **フラグの提出**
    - <del>**エンドポイント**: `POST /api/challenges/:challengeId/submit`</del>
    - **ペイロード**: `{ "flag": "ユーザーが提出したフラグ" }`
    - **目的**: ユーザーが提出したフラグが正解かどうかを検証し、結果を記録します。

- **公開されているチャレンジ一覧の取得**
    - <del> **エンドポイント**: `GET /api/public/challenges` </del>
    - **目的**: 認証されていないユーザーでもアクセスできる、公開中のチャレンジのリストを返します。ページネーションやフィルタリングをサポートすると便利です。

- **特定のチャレンジ詳細の取得（解答者用）**
    - <del>**エンドポイント**: `GET /api/public/challenges/:challengeId`<del>
